...
- translucentWindowsCompat: false
  ...
  $description: >
    Enable ONLY if you use the "Translucent Windows" Windhawk mod.
    Reverts to the legacy compositing path (AlphaBlend), disabling
    per-pixel alpha rendering and background styling changes.
    Do NOT enable this if you use TranslucentFlyouts — it is a
    different program that works with the default mode. Enabling
    this with TranslucentFlyouts + Dark Mode Context Menus will
    cause a white flash.
  $description:pt: >
    Ative APENAS se você usa o mod "Translucent Windows" do Windhawk.
    Reverte para o caminho de composição legado (AlphaBlend), desativando
    a renderização alfa por pixel e as alterações de estilo de fundo.
    NÃO ative se você usa o TranslucentFlyouts — é um programa
    diferente que funciona com o modo padrão. Ativar com
    TranslucentFlyouts + Dark Mode Context Menus causa um flash branco.
  $description:es: >
    Actívalo SOLO si usas el mod "Translucent Windows" de Windhawk.
    Revierte al camino de composición heredado (AlphaBlend), desactivando
    la composición alfa por píxel y los cambios de estilo de fondo.
    NO lo actives si usas TranslucentFlyouts — es un programa
    diferente que funciona con el modo predeterminado. Activarlo con
    TranslucentFlyouts + Dark Mode Context Menus causa un destello blanco.
*/
// ==/WindhawkModSettings==
...
If you use the **Translucent Windows** Windhawk mod alongside this one, enable the
...
> **Note:** If you use **TranslucentFlyouts** (a standalone application), do **NOT**
> enable this setting — TranslucentFlyouts uses DWM backdrop effects and works
> correctly with the default mode. The compatibility setting is exclusively for the
> Translucent Windows Windhawk mod.
...
• (1.0.2) Eliminated SLWA→ULW transition flash, fixing compatibility with TranslucentFlyouts + Dark Mode Context Menus
...
Se você usa o mod **Translucent Windows** do Windhawk junto com este, ative a
configuração **Modo de compatibilidade com Translucent Windows**. Isso reverte
a renderização da animação para o caminho de composição legado (AlphaBlend),
desativando a transparência alfa por pixel e as alterações de estilo de fundo.
As animações de slide e fade continuam funcionando normalmente — apenas o método
de composição muda.

> **Observação:** Se você usa o **TranslucentFlyouts** (aplicativo independente),
> **NÃO** ative essa configuração — o TranslucentFlyouts usa efeitos de backdrop
> DWM e funciona corretamente com o modo padrão. A configuração de compatibilidade
> é exclusivamente para o mod Translucent Windows do Windhawk.
...
• (1.0.2) Eliminação do flash da transição SLWA→ULW, corrigindo compatibilidade com TranslucentFlyouts + Dark Mode Context Menus
...
Si usas el mod **Translucent Windows** de Windhawk junto con este, activa la opción
...
> **Nota:** Si usas **TranslucentFlyouts** (aplicación independiente), **NO**
> actives esta opción — TranslucentFlyouts usa efectos de backdrop DWM y funciona
> correctamente con el modo predeterminado. La opción de compatibilidad es
> exclusivamente para el mod Translucent Windows de Windhawk.
...
• (1.0.2) Eliminación del destello de la transición SLWA→ULW, corrigiendo compatibilidad con TranslucentFlyouts + Dark Mode Context Menus
*/
// ==/WindhawkModReadme==
    ...
    UINT_PTR uIDSubclass, DWORD_PTR dwRefData)
{
    ...
    // -----------------------------------------------------------------
    {
        ...
        // em ResetMNAnimateInfo ao fim (natural ou por cancelamento).
        {
            ...
            dwEx |=  WS_EX_LAYERED;     // habilita transparência
            SetWindowLongW(hwnd, GWL_EXSTYLE, dwEx);

            // Modo de compatibilidade (SLWA): define alpha=0 via SLWA para
            // ocultar a janela antes da primeira pintura.
            //
            // Modo ULW: NÃO chamamos SLWA aqui. Uma janela WS_EX_LAYERED
            // sem SLWA nem ULW é tratada como totalmente transparente pelo
            // DWM — invisível por padrão. Isso evita o ciclo SLWA→ULW
            // (remover/re-adicionar WS_EX_LAYERED) que causava flash branco
            // quando usado com TranslucentFlyouts + Dark Mode Context Menus.
            if (g_bTranslucentCompat)
    ...
    // -----------------------------------------------------------------
    {
        ...
        if (!g_bTranslucentCompat)
        {
            // Modo ULW: a janela foi marcada como WS_EX_LAYERED em
            // WM_WINDOWPOSCHANGING sem chamar SLWA — já está pronta
            // para UpdateLayeredWindow. A primeira chamada a MNAnimate(true)
            // abaixo estabelece o modo ULW com o primeiro frame da animação.
            //
            // A borda NC do DWM é suprimida via WM_NCPAINT durante a
            // animação e restaurada pelo RDW_FRAME em ResetMNAnimateInfo.
        }
